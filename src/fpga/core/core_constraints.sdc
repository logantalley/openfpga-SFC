...
# (direct-stream: the former ss_dout -> ddr_di false path is REMOVED.  With
# MCLK ungated the engine and the stream module share clk_sys, so that bus is
# an ordinary synchronous path that must be timed, not cut.)
