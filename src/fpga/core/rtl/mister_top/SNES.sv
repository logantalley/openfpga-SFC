module MAIN_SNES (
  ...
  // ACLK is left free-running (see .ACLK below): gating it during the serve
  // broke the Pocket's continuous-audio stream and made the save FAIL.
  //
  ...
  main #(
      ...
      .ACLK(clk_sys),     // audio clock free-running (gating it for the ~120 ms
                          // serve broke the Pocket's continuous-audio stream →
                          // "save failed"; must stay free)
      ...
      // Save states — DDR interface (to save_state_controller via core_top)
      .SS_SAVE    (ss_save),
      .SS_LOAD    (ss_load),
      .SS_DDR_DI  (ss_din),
      .SS_DDR_ACK (ss_ack),
...
`endif
  ...
  // The CDC synchronizers + clk_mem-side SDRAM-access FSM were extracted
  // verbatim into ss_sdram_arbiter.sv so the identical RTL can also be
  // instantiated by the focused ModelSim testbench (tb_ss_staging.sv).
  wire        ss_loading_mem;
  wire        ss_mem_rd;
  wire        ss_mem_wr;
  ...
  ss_sdram_arbiter ss_arbiter (
      ...
      .sdram_dout    (ROM_Q_SDRAM),
      .sdram_busy    (sdram_busy)
  );

  // SDRAM input mux.  When ss_loading_mem is high (Phase C only), the
  // savestate FSM drives SDRAM.  Otherwise behavior is identical to the
  // previous direct wiring.  cart_download keeps priority for safety.
  wire [24:0] sdram_addr_mux =
      cart_download   ? ioctl_addr :
      ss_loading_mem  ? ss_mem_addr :
                        ROM_ADDR;
  wire [15:0] sdram_din_mux =
      cart_download   ? ioctl_dout :
      ss_loading_mem  ? ss_mem_din :
                        ROM_D;
  wire        sdram_rd_mux =
      cart_download   ? 1'b0 :
      ss_loading_mem  ? ss_mem_rd :
                        (RESET_N ? ~ROM_OE_N : RFSH);
  wire        sdram_wr_mux =
      cart_download   ? ioctl_wr :
      ss_loading_mem  ? ss_mem_wr :
                        ~ROM_WE_N;
  wire        sdram_word_mux =
      cart_download   ? 1'b1 :
      ss_loading_mem  ? 1'b1 :
